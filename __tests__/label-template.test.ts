...
^XZ`
  ...
  test('should support withOptions syntax for DPI and units', () => {
    const label = labelTemplate.withOptions({ dpi: 300, units: 'mm' })`^XA
...
^XZ`
  ...
  test('should preserve whitespace exactly as provided', () => {
    const label = labelTemplate`
      ^XA
      
      ^FO50,50^A0N,28,28^FD Text With Spaces ^FS
...
^BY5,2,270
^FO100,550^BC^FD${trackingNumber}^FS
^FX Added via fluent API^FO50,800^AAN,28,28^FDProcessed via fluent-zpl^FS^XZ`
